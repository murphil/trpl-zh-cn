...
浮点数表示采用 IEEE-754 标准。`f32` 是单精度浮点数，`f64` 是双精度浮点数。

#### 数字运算符

Rust 支持数字类型的基本数学运算：加、减、乘、除以及取余。以下代码展示了如何使用他们：
...
这些语句中的表达式，使用数学运算符计算出了一个值，然后绑定到一个变量。附录 B 中有 Rust 提供的所有运算符。

#### 布尔型

和多数编程语言一样，Rust 中的布尔类型有两个可能的值：`true` 和 `false`，使用 `bool` 表示。例如：
...
布尔值的主要使用场景是条件表达式，例如 `if`（“控制流”部分会详细讲解）。

#### 字符类型

目前为止只使用到了数字，不过 Rust 也支持字符。Rust 的 `char` 类型是大部分语言中的基本字符类型，示例如下：
...
Rust 的 `char` 类型代表了一个 Unicode 标量值（Unicode Scalar Value），这意味着它可以比 ASCII 表示更多内容。拼音字母（Accented letters），中文/日文/汉语等象形文字，emoji（絵文字）以及零长度的空白字符对于 `char` 类型都是有效的。Unicode 标量值包含从 `U+0000` 到 `U+D7FF` 和 `U+E000` 到 `U+10FFFF` 之间的值。不过，“字符”并不是一个 Unicode 中的概念，所以人直觉上的“字符”可能与 Rust 中的  `char` 并不相符。第八章“字符串”部分将详细讨论这个主题。
...
`tup` 变量绑定了整个元组，因为元组被认为是一个单独的复合元素。要从元组中获取值，可以使用模式匹配（pattern matching）来解构（destructure ）元组，像这样：
...
程序首先创建了一个元组并绑定到 `tup` 变量上。接着使用了 `let` 和一个模式将 `tup` 分解为三个不同的变量，`x`、`y` 和 `z`。这叫做*解构*（*destructuring*），因为它将一个元组拆成了三个部分。最后，程序打印出了 `y` 的值，也就是 `6.4`。

除了使用模式匹配来解构，也可以使用点号（`.`）后跟值的索引来直接访问他们。例如：
...
这里创建了一个元组，`x`，并接着使用索引为每个元素创建新变量。跟大多数编程语言一样，元组的第一个索引值是 0。

#### 数组

另一种获取多个值集合的方式是**数组**（*array*）。与元组不同，数组中的每个元素的类型必须相同。Rust 中的数组与一些其他语言中的数组不同，因为 Rust 中的数组是固定长度的：一旦声明，他们的长度不能增长或缩小。
